...
class FirebaseImageStorageService implements ImageStorageService {
  ...
  Future<void> deleteImage(String storagePath) async {
    try {
      ...
      if (e.code == 'object-not-found') {
        _logger.w('Image not found, considering deletion successful: $storagePath');
        return;
      }
  ...
  Future<String> getDownloadUrl(String storagePath) async {
    try {
      ...
      _logger.e('Firebase error getting download URL: ${e.code} - ${e.message}');
  ...
  Exception _handleFirebaseStorageError(FirebaseException e, String operation) {
    switch (e.code) {
      ...
      case 'unauthorized':
        return Exception('Unauthorized access to storage. Please check your permissions.');
      case 'canceled':
        return Exception('$operation operation was canceled');
      case 'unknown':
        return Exception('An unknown error occurred during $operation');
      case 'retry-limit-exceeded':
        return Exception('Upload failed after multiple retries. Please check your connection.');
